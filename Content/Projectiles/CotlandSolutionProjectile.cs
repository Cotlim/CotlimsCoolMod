            ...
            for (int k = i - size; k <= i + size; k++)
            {
                for (int l = j - size; l <= j + size; l++)
                {
                    if (WorldGen.InWorld(k, l, 1) && Math.Abs(k - i) + Math.Abs(l - j) < Math.Sqrt(size * size + size * size))
                    {
                        ...
                        if (wall != 0 && wall != ModContent.WallType<CotlandWallUnsafe>())
                        {
                            if (wall == WallID.SpiderUnsafe)
                                Main.tile[k, l].WallType = (ushort)ModContent.WallType<CotlandWallUnsafe>();
                            else
                                Main.tile[k, l].WallType = (ushort)ModContent.WallType<CotlandWall>();
