...
{
    public class CotShotgun : ModItem
	{
		public override void SetDefaults() {
			...
			// Gun Properties
			Item.shoot = ModContent.ProjectileType<FasterStarLineProjectile>(); // For some reason, all the guns in the vanilla source have this.
		...
		public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback) 
		{
			...
			for (int i = 0; i < NumProjectiles; i++) {
                ...
                float rotation = velocity.RotatedBy(MathHelper.ToRadians(5 * (i - 10)*player.direction)).ToRotation();

				Projectile.NewProjectileDirect(source, player.position, Vector2.Zero, type, damage, knockback, player.whoAmI, 
					ai0:i*1.5f, ai1:rotation, ai2:velocity.Length());
			}
